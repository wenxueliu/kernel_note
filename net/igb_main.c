 ...
 *  return -ENOMEM.
 *
 *  为 adapter->q_vectors 分配空间并初始化。
 *  先为 rx 分配 num_rx_queues 个 q_vector, 其中 ring 为 1, 只有 rx
 *  之后为 tx 分配 num_tx_queues 个 q_vector, 其中 ring 为 2, 一个 rx, 一个 tx
 *
 *  每个 napi 的 poll 为 igb_poll, weight 为 64, 并将 nap->dev_list 加入 adapter->netdev->dev_list.
 *
 *  rx 的 ring 的 count 为 adapter->rx_ring_count
 *  tx 的 ring 的 count 为 adapter->tx_ring_count
 *
 *  adapter->tx_ring 保持所有 q_vectors 中 tx 的 ring
 *  adapter->rx_ring 保持所有 q_vectors 中 rx 的 ring
 **/
...
/*
 * 设置 adapter->rss_queues 及 adapter->flags 是否包含 IGB_FLAG_QUEUE_PAIRS;
 */

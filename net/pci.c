 ...
 * 3. 打开传输开关
 *
 * 如果 dev 的 msi 或 msix 打开, 表面设备已经 enable, 返回 0.
 * 否则, 配置 dev 的 PCI_INTERRUPT_PIN 为 1, 设置 dev 对应字段 ~PCI_INTERRUPT_PIN 为 0
 *
 */
static int do_pci_enable_device(struct pci_dev *dev, int bars)
{
    ...
    //dev->bus->ops->read(dev->bus, dev->devfn, PCI_INTERRUPT_PIN, len, pin)

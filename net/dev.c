 ...
 *
 *      增加 pt->list 到 head 后面, 具体为 根据 pt->type 来决定增加
 *      pt->list 到 pt->dev->ptype_all, ptype_all, pt->dev->ptype_specific
 *      还是 pt->dev->ptype_all 后面
 */
...
{
        /*
         *  如果 skb_shinfo(skb)->tx_flags 没有设置 SKBTX_DEV_ZEROCOPY, 返回 0
         *  如果 skb_shinfo(skb)->tx_flags 设置 SKBTX_DEV_ZEROCOPY, 将 skb_shinfo(skb)->frags 中的元素用内核分配的页替换.
         *  成功返回 0, 如果内核内存不够, 返回 -ENOMEM 错误.
         */
        if (unlikely(skb_orphan_frags(skb, GFP_ATOMIC)))
                return -ENOMEM;
        atomic_inc(&skb->users);
        /*
         * 如果是 ip 协议, 调用 ip_rcv. 具体参考 net/ipv4/af_inet.c 中 inet_init 函数
         */
